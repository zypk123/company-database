...
public class OverRunCheckPointDto {
	    ...
	    public OverRunCheckPoint parseToOverRunCheckPointDto(){
	    	OverRunCheckPoint overRunCheckPoint=new OverRunCheckPoint();
	    	ObjectMapUtils.parseObject(overRunCheckPoint, this);
	    	if (!StringUtil.isNullOrEmpty(this.getLon()) && !StringUtil.isNullOrEmpty(this.getLat())) {
	    		overRunCheckPoint.setLonLat("POINT(" + this.getLon().trim() + " " + this.getLat().trim() + ")");
			}
	    	else{
	    		overRunCheckPoint.setLonLat(null);
	    	}
	    	return overRunCheckPoint;
	    }
}
