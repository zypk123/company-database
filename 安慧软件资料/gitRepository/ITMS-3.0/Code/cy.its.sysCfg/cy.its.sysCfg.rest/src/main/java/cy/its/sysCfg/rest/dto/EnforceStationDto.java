...
public class EnforceStationDto {
    ...
    public EnforceStation parseToEnforceStation(){
    	EnforceStation enforceStation=new EnforceStation();
    	ObjectMapUtils.parseObject(enforceStation, this);
    	if (!StringUtil.isNullOrEmpty(this.getLon()) && !StringUtil.isNullOrEmpty(this.getLat())) {
    		enforceStation.setLonLat("POINT(" + this.getLon().trim() + " " + this.getLat().trim() + ")");
		}
    	else{
    		enforceStation.setLonLat(null);
    	}
    	return enforceStation;
    }
}
