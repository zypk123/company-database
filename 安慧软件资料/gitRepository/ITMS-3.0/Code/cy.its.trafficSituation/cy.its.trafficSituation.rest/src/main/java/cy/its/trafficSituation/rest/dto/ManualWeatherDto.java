/**
 * @Title: ManualWeatherDto.java
 * @Package cy.its.trafficSituation.rest.dto
 * @Description: 人工登记恶劣天气dto
 * @author gyf guanyf@cychina.cn
 * @date 2015年11月10日 下午3:36:16
 * @version V1.0
 * @Revision : $Rev$
 * @Id: $Id$
 *
...
public class ManualWeatherDto extends BaseDto {
	...
	/**
	 * setter method
	 * @param badWeatherId the badWeatherId to set
	 */
	...
	/**
	 * setter method
	 * @param meteorologyType the meteorologyType to set
	 */
	...
	public ManualWeatherDto(){
		
	}
	public ManualWeatherDto(TrafficManualWeather weather){
		SimpleDateFormat sdf=new SimpleDateFormat("yyyy-MM-dd HH:mm:ss");
		ObjectMapUtils.parseObject(this, weather);
		if(weather.getReportTime() != null){
			this.setReportTime(sdf.format(weather.getReportTime()));
		}
		if(weather.getLonLat() != null){
			String tempStr=weather.getLonLat().trim();
	...
	public TrafficManualWeather parseToTrafficManualWeather() throws ParseException{
		TrafficManualWeather weather= new TrafficManualWeather();
		ObjectMapUtils.parseObject(weather, this);
		if(this.getLon()!=null&& this.getLat()!=null){
			weather.setLonLat("POINT("+this.getLon().trim()+" "+this.getLat().trim()+")");
		}
		else{
			weather.setLonLat(null);
		}
		weather.setReportTime(new Date());
		weather.setReportBy(this.getCurrentUserName());
		return weather;
	}
