...
class OraDiscardedDbError implements IDiscardedDbError {
	...
	OraDiscardedDbError() {
		...
		Error.put(2291, "违法外键约束");
		Error.put(24381, "数组 DML 出错");

	}

	@Override
	public Boolean isDiscardedDataError(Throwable e) {
		return isDiscarded(e, null);
	}

	@Override
	public boolean isDiscardedDataErrorExcludeUniqueConstraint(Throwable e) {
		return isDiscarded(e, (errCode) -> errCode == ConstValue.INT_1);
	}

	private boolean isDiscarded(Throwable e, Predicate<Integer> filter) {
