 ...
 * @version V1.0
 */
public class DeviceFaultSocket extends WebscoketServiceImplA implements IWebscoketReceiver {

	private Log4jFactoryProxy log = Log4jFactoryProxy.getSingleton(DeviceFaultSocket.class);

	private final String routingKey = "its_alarm_device";
	/**
	 * 两个KEY可以是同一个
	 */
	private final String monitorKey = "deviceFault";

	private DeviceFaultSocket() {
	...
	public Boolean receive(JSONObject jsonObject) {
		...
		listbean.stream().forEach(wsb -> {
			...
			if(orgPrivCode != null && !orgPrivCode.isEmpty()
					&& "1".equals(jsonObject.getString("alarmType")) &&  jsonObject.getString("orgPrivilegeCode").startsWith(orgPrivCode)){
