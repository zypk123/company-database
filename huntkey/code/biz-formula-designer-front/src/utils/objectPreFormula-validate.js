 ...
 */
const ObjectPreFormulaValidateUtils = {
  ...
  ananalyticFormula: function (formulaArrayPara, defineCustomFuncs, exceptParaType, exceptParaName) {
    ...
    console.log('自定义函数列表', defineCustomFuncs)
    console.log('属性公式类型', exceptParaType)
    console.log('属性公式名称', exceptParaName)
    ...
    if (quotesFlag === '0') {
      ...
      if (funcStack.length === 1) {
        ...
        if (this.returnTypeCompare([exceptParaType], funcStack[0].parasStack[0].paraType)) {
  ...
  /*
   PPI不需要校验公式返回类型与PPI的公式期待的类型一致
   */
  returnTypeCompare: function (types, paraType) {
    return true
    // paraType = paraType.toUpperCase()
    // types = types.map((type) => type.toUpperCase())
    // if (['TEXT', 'VARCHAR'].includes(paraType)) {
...
export default ObjectPreFormulaValidateUtils
