...
const Utils = {
  ...
  clonePropsDeep: function (target, source, mode = 0) {
    // mode = 0 从src往target中克隆值 随便干
    // mode = 1 从src往target中克隆值, 只有target存在该index时才clone值
    // mode = 2 从src往target中克隆值, 只有target存在该index时,且改index的值为空(''或null)时 才clone值
    for (let index in source) {
      if (mode === 1) {
         // 从src往target中克隆值, 只有target存在该index时才clone值 clonePropsDeep
        if (target[index] !== undefined) {
          target[index] = this.cloneDeep(source[index])
        }
      } else if (mode === 2) {
         // 从src往target中克隆值, 只有target存在该index时,且改index的值为空(''或null)时 才clone值
        if (target[index] !== undefined) {
          if (!target[index]) {
            target[index] = this.cloneDeep(source[index])
          }
