<template>
  <div class="rel-trigger-condition-config" v-loading.fullscreen.lock="pageConfig.isLoading" :element-loading-text="pageConfig.loadingText">
    ...
    <el-row>
      <el-col class="common-vgap" :span="24">
        <span>属性:</span><span class="prop-full-lable">{{clssName + '.' + clssPropName}}</span>
      </el-col>
...
import RelTriggerConditionCondEdit from './RelTriggerConditionCondEdit'
export default {
  name: 'relTriggerConditionConfig',
  data () {
    return {
      pageConfig: {
        isLoading: false,
  ...
  // 页面初始化之前调用该方法
  created () {
    if (this.$route.name === 'relTirggerConditionConfig') { // 路由参数赋值
      this.pageConfig.isLoading = true
      this.propId = this.$route.params.propId
      this.prplId = this.$route.params.prplId
      console.log(this.propId + '====' + this.prplId)
      // 初始化属性下拉框
      relConditionService.queryBaseInfoOfRelCondition({propId: this.propId}, (res) => {
        console.log('res', res)
        this.clssName = (res.edmpEdmcName === null ? '' : res.edmpEdmcName)
        this.clssNameEn = (res.edmpEdmcNameEn === null ? '' : res.edmpEdmcNameEn)
        this.clssId = (res.edmpEdmcId === null ? '' : res.edmpEdmcId)
        this.getPropsbyClassId(res.edmpEdmcId).then((resData) => {
          ...
          for (let i = 0; i < this.tempArray.length; i++) {
            if (this.propId === this.tempArray[i].id) {
              this.propCode = this.tempArray[i].casEdmpCode
              this.clssPropName = this.tempArray[i].casEdmpName
            }
          }
        })
        this.getPropsbyClassId(res.edmpEdmcId).then((resData) => {
  ...
  methods: {
    ...
    queryRelTirggerConditionAndConditions () {
      relConditionService.initRelTriggerCond({propId: this.propId}, {prplId: this.prplId}, (res) => {
        console.log('r', res)
        if (res && res.relatedProperty && res.relatedProperty.prplId) {
          ...
          // this.pageConfig.loadingText = '初始化触发条件失败，没有找到该属性！'
          this.pageConfig.isLoading = false
        }
      }, (error) => {
    ...
    saveAll () {
      this.$refs.editor.valid().then(() => {
        ...
        let relatedProperty = {
          prplId: this.prplId,
          prplClassRelatedFrom: this.clssName + '.' + this.clssPropName,
          prplClassRelatedTo: '',
        ...
        relConditionService.saveTriCon({relatedProperty, relatedConditions, propId}, (res) => {
          this.prplId = res
    ...
    openCondEditDialog ({index, title, cond, mode}) {
      this.OpenGlobalDialog({
        name: 'relTriggerConditionCondEdit',
        component: RelTriggerConditionCondEdit,
        title: title,
        props: {
          index: index,
          mode: mode,
        ...
        events: {
          ...
          close: () => {
            this.CloseGlobalDialog('relTriggerConditionCondEdit')
          }
        }
      })
    },
    ...
    delRelTrigger  () {
      if (!this.prplId) {
        ...
        this.AppUtils.showSuccess('还没有触发条件呢,请添加后再重试...')
        return
      }
