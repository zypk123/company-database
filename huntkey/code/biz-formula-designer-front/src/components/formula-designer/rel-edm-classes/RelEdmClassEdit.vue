    ...
    <!--       <el-form-item v-if="pageConfig.isSelectingEdmClass">
      ...
      <el-col class="common-vgap" :span="24">
        <el-table
          ...
          <el-table-column prop="cndtOperate" label="条件" header-align="center" align="center" width="150" :formatter="tableCndrOperateFormat">
          </el-table-column>
...
import RelEdmClassCondEdit from './RelEdmClassCondEdit'
export default {
          ...
          {required: true, message: '相关类名称不能为空'},
          {
            validator: (rules, value, callback) => {
              if (!this.Utils.validate(value, 'name_en')) {
                ...
                callback(new Error('格式不正确（字母开头,英文和数字,不能包含空格）'))
              }
              callback()
            }
          }
  ...
  created () {
    ...
    this.Utils.clonePropsWhenExist(this.oldRelEdmClassInfo, this.initData.relClass)
    this.Utils.clonePropsWhenExist(this.newRelEdmClassInfo, this.initData.relClass)
  ...
  methods: {
    tableCndrOperateFormat (row, column) {
      ...
      if (cndrOperate === undefined) {
        ...
        for (let i = 0; i < this.cndtOperatorDict.length; i++) {
          if (cndrOperate === this.cndtOperatorDict[i].val) {
    ...
    lockSelectedEdmClass () {
      ...
      this.$refs.relClassNameForm.validate((valid) => {
        ...
        if (valid) {
          ...
          if (needResetCond) {
            // 切换类了, 需清空关联条件和关联公式
            this.$confirm('变更相关类会清空关联条件和关联公式,是否确认？', '提示', {
    ...
    updateRelClassInfo4Lock (needResetCond) {
      ...
      this.updateRelClassInfo().then(() => {
        ...
        this.AppUtils.showSuccess('相关类锁定成功')
      }).catch((error) => {
    ...
    openCondEditDialog ({index, title, cond, mode}) {
      this.OpenGlobalDialog({
        name: 'RelEdmClassCondEditDlg',
        component: RelEdmClassCondEdit,
        props: {
          store: this.initData.store,
        ...
        events: {
          ...
          close: () => {
            this.CloseGlobalDialog('RelEdmClassCondEditDlg')
          }
        }
      })
    },
    saveCond ({index, condData}) {
      ...
      this.$set(this.oldConds, index, condData)
    },
    deleteCond (index) {
      // this.$set()
    ...
    updateRelClassInfo () {
      return new Promise((resolve, reject) => {
        ...
        formulaService.saveOrUpdateClassRelated(
          {
            classRelated: this.newRelEdmClassInfo,
            tplConditions: this.oldConds
          },
          (resData) => {
            this.lastEdmcId = this.newRelEdmClassInfo.id
            this.Utils.clonePropsWhenExist(this.oldRelEdmClassInfo, this.newRelEdmClassInfo)
            this.$emit('relClassUpdated')
        ...
        ) // end of formulaService.saveOrUpdateClassRelated
