 ...
 */
@Component(value = "hbaseValidate")
public class HbaseDataValidate implements DataValidate {

    private static Logger logger = LoggerFactory.getLogger(HbaseDataValidate.class);
    ...
    public boolean validate4search(JSONObject serviceData) {
        ...
        // 2. 校验分页参数是否为数字
        JSONObject pagenationJson = serviceData.getJSONObject(NodeConstant.PAGENATION);
        boolean pageflag = validatePagenation(pagenationJson);
        if (!pageflag) {
            return false;
        }
        // 3. 校验columns的编码是否有效；
        JSONArray columnArray = serviceData.getJSONArray(NodeConstant.COLUMNS);
        boolean columnflag = validateColumns(columnArray, edmName);
        if (!columnflag) {
            return false;
        }
        // 4. 校验conditions的编码是否有效
        JSONArray conditionarray = serviceData.getJSONArray(NodeConstant.CONDITIONS);
        boolean conditionflag = validateConditions(conditionarray, edmName);
        if (!conditionflag) {
            return false;
        }
        // 5. 校验conditions的编码是否在索引范围之内
        boolean conditionsindex = validateConditionsIsIndex(conditionarray, edmName);
        if (!conditionsindex) {
            return false;
        }

        // 6. 如果包含orderBy，判断attr的编码是否有效
        JSONArray jsonorderby = serviceData.getJSONArray(NodeConstant.ORDERBY);
    ...
    private Set<String> filterSystemKeys(Collection<String> keys) {
        ...
        for (Iterator<String> iterator = keys.iterator(); iterator.hasNext(); ) {
            ...
            if (NodeConstant.ID.equals(_key) || NodeConstant.PID.equals(_key)
    ...
    public boolean validateConditionsIsIndex(JSONArray conditionArray, String edmName) {
        ...
        for (Iterator<String> iterator = _indexes.iterator(); iterator.hasNext(); ) {
            ...
            if (!fieldsList.contains(_index)) {
                logger.info("{0} is not an index attribute and es not index!", _index);
                throw new IllegalArgumentException(_index + " is not an index attribute and es not index!");
            }
        }
