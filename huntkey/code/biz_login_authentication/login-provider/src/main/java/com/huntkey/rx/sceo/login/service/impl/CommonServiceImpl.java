...
public class CommonServiceImpl implements CommonService {
    ...
    public Result sendMsg(String tel, String content) {
        ...
        try {
            ...
            result.setErrMsg(msgStatusCode.getErrorMsg() + content);
        } catch (IOException e) {
    ...
    public Result uploadFile(MultipartFile file, String fileName) {
        ...
        try {
            ...
            String fileExtName = fileName.substring(indexOf + 1);
            StorePath path = defaultFastFileStorageClient.uploadFile(file.getInputStream(), file.getSize(),
                    fileExtName, new HashSet<MateData>());
            result.setData(path.getFullPath());
        } catch (Exception e) {
            logger.error("uploadFile error : " + e.getMessage(), e);
            result.setRetCode(Result.RECODE_ERROR);
            result.setErrMsg("uploadFile error :" + e.getMessage());
        }
        return result;
    }
}
