...
public class MagnetController {
     ...
     * @return
     */
    @RequestMapping(value = "/del/magnetSetList/{magnetId}", method = RequestMethod.DELETE)
    @MethodRegister(
            edmClass = "magnet",
            methodCate = "ID系统",
            methodDesc = "批量删除磁贴属性集"
    )
    public Result delMagnetSetList(@PathVariable String magnetId) {
        Result result = null;
        try {
            result = magnetService.delMagnetSetList(magnetId);
        } catch (Exception e) {
            logger.error("批量删除磁贴属性集：" + e.getMessage(), e);
            throw new RuntimeException("批量删除磁贴属性集：" + e.getMessage(), e);
        }
        return result;
    }
