...
public class LoginController {
    ...
    private void updateEcosystemSession(PeopleEntity dbuserInfo, String token) {
        ...
        String redisTimeout[] = redisTimeoutSeconds.split(" ");
        logger.info("存入redis中的key为： " + token);
        RedisUtils.getInstance().setValue(tokenKey.getBytes(), SerializeUtils.serialize(ecosystemSession),
                Integer.parseInt(redisTimeout[0]) * Integer.parseInt(redisTimeout[1]) * Integer.parseInt(redisTimeout[2]) * Integer.parseInt(redisTimeout[3]));
    ...
    @MethodRegister(
            edmClass = "people",
            methodCate = "ID系统",
            methodDesc = "checkCardNoAndTel"
    )
    @RequestMapping(value = "/checkCardNoAndTel", method = RequestMethod.GET)
    public Result checkCardNoAndTel(@RequestParam String epeoCardNo, @RequestParam String epeoTel) {
        return loginService.checkCardNoAndTel(epeoCardNo, epeoTel);
    }
    ...
    @MethodRegister(
            edmClass = "people",
            methodCate = "ID系统",
            methodDesc = "retrievePass"
    )
    @LimitIpRequest(
            methodUrl = "/login/retrievePass",
            limitCount = 5
    )
    @RequestMapping(value = "/retrievePass", method = RequestMethod.POST)
    public Result retrievePass(@RequestBody RetrievePassVo retrievePassVo) {
        return loginService.retrievePass(retrievePassVo);
    }
