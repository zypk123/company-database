...
public class UserServiceImpl implements UserService {
    ...
    public Result updatePeopleInfo(PeopleEntity peopleEntity) {
        ...
        try {
            ...
            result = preHandlePeople(map, strs, result);
            if (!Result.RECODE_SUCCESS.equals(result.getRetCode())) {
                return result;
            }
            peopleEntity.setModuser(UserUtil.getUserId());
            peopleEntity.setEpeo_tel(null);
            peopleEntity.setEpeo_mail(null);
            peopleEntity.setEpeo_card_no(null);
            peopleEntity.setEpeo_fist_name(null);
            peopleEntity.setEpeo_last_name(null);
            peopleEntity.setEpeo_password(null);
            int i = ormService.updateSelective(StringUtils.preUpdateEntity(peopleEntity));
            result.setData(i);
        } catch (Exception e) {
    ...
    @Override
    @DynamicDataSource(isDefaultSource = true)
    public Result certification(UserInfo userInfo) {
        Result result = new Result();
        try {
            if (StringUtil.isNullOrEmpty(userInfo.getEpeoCardNo())) {
            ...
            String[] strs = {"身份证号"};
            Map<String, String> map = new LinkedHashMap<String, String>();
            map.put("epeo_card_no", userInfo.getEpeoCardNo());
            result = preHandlePeople(map, strs, result);
            if (!Result.RECODE_SUCCESS.equals(result.getRetCode())) {
                return result;
            }
            ...
            peopleEntity.setEpeo_fist_name(userInfo.getEpeoFistName());
            peopleEntity.setEpeo_last_name(userInfo.getEpeoLastName());
            peopleEntity.setEpeo_card_no(userInfo.getEpeoCardNo());
            peopleEntity.setModuser(UserUtil.getUserId());
            result.setData(ormService.updateSelective(peopleEntity));
        } catch (Exception e) {
            logger.error("certification error : " + e.getMessage(), e);
            result.setRetCode(Result.RECODE_ERROR);
            result.setErrMsg("certification error :" + e.getMessage());
        }
        return result;
    }
     ...
     * @return
     */
    private Result preHandlePeople(Map<String, String> map, String[] strs, Result result) throws Exception {
        ...
        while (iterator.hasNext()) {
            ...
            OrmParam ormParam = new OrmParam();
            String whereExp = OrmParam.and(ormParam.getEqualXML(e.getKey(), e.getValue()));
            ormParam.setWhereExp(whereExp);
            ...
            boolean sizeE1 = peopleEntities.size() == 1 && !UserUtil.getUserId().equals(peopleEntities.get(0).getId());
            if (sizeE1 || peopleEntities.size() > 1) {
                result.setRetCode(Result.RECODE_ERROR);
    ...
    public Result addPeople(PeopleEntity peopleEntity, String enterpriseId) {
        ...
        try {
            ...
            String peopId = (String) ormService.insertSelective(StringUtils.preInstallEntity(peopleEntity));
    ...
    public Result addStuSet(EpeoEpeoStuSetaEntity stuSet) {
        ...
        try {
            ...
            result.setData(ormService.insertSelective(StringUtils.preInstallEntity(stuSet)));
        } catch (Exception e) {
    ...
    public Result addWorkSet(EpeoEpeoWorkSetaEntity workSet) {
        ...
        try {
            ...
            result.setData(ormService.insertSelective(StringUtils.preInstallEntity(workSet)));
        } catch (Exception e) {
    ...
    public Result modWorkSet(EpeoEpeoWorkSetaEntity workSet) {
        ...
        try {
            workSet.setPid(UserUtil.getUserId());
            workSet.setClassName("people");
            result.setData(ormService.update(StringUtils.preUpdateEntity(workSet)));
        } catch (Exception e) {
    ...
    public Result modStuSet(EpeoEpeoStuSetaEntity stuSet) {
        ...
        try {
            stuSet.setPid(UserUtil.getUserId());
            stuSet.setClassName("people");
            result.setData(ormService.update(StringUtils.preUpdateEntity(stuSet)));
        } catch (Exception e) {
    ...
    public Result addPeopleEnteSet(String userId, String enterpriseId) {
        ...
        try {
            ...
            epeoEpeoEnteSetaEntity.setClassName("people");
            epeoEpeoEnteSetaEntity.setEpeo_ente_obj(enterpriseId);
            String peopEpeoEnteSetaId = (String) ormService.insertSelective(StringUtils.preInstallEntity(epeoEpeoEnteSetaEntity));
            logger.info("将用户Id和企业Id插入到关联表里面，返回的关联表ID为" + peopEpeoEnteSetaId);
    ...
    public Result modFamSet(EpeoEpeoFamSetaEntity famSet) {
        ...
        try {
            ...
            result.setData(ormService.updateSelective(StringUtils.preUpdateEntity(famSet)));
        } catch (Exception e) {
    ...
    public Result addFamSet(EpeoEpeoFamSetaEntity famSet) {
        ...
        try {
            ...
            result.setData(ormService.insertSelective(StringUtils.preInstallEntity(famSet)));
        } catch (Exception e) {
    ...
    public Result getStuSetList() {
        ...
        try {
            OrmParam param = new OrmParam();
            param.setWhereExp(OrmParam.and(param.getEqualXML("pid", UserUtil.getUserId())));
            param.setOrderExp(SQLSortEnum.DESC, "epeo_stu_beg");
            List<EpeoEpeoStuSetaEntity> epeoEpeoStuSetaEntities = ormService.selectBeanList(EpeoEpeoStuSetaEntity.class, param);
    ...
    public Result getWorkSetList() {
        ...
        try {
            OrmParam param = new OrmParam();
            param.setWhereExp(OrmParam.and(param.getEqualXML("pid", UserUtil.getUserId())));
            param.setOrderExp(SQLSortEnum.DESC, "epeo_work_beg");
            List<EpeoEpeoWorkSetaEntity> workSetaEntities = ormService.selectBeanList(EpeoEpeoWorkSetaEntity.class, param);
    ...
    public Result modTel(UserInfo userInfo) {
        ...
        try {
            ...
            result.setData(ormService.updateSelective(StringUtils.preUpdateEntity(peopleEntity)));
        } catch (Exception e) {
    ...
    public Result modPassWord(UserInfo userInfo) {
        ...
        try {
            ...
            String encryNewPw = EncryptUtil.encryptPassWord(userInfo.getNewPassWord());
            if (peopleEntity.getEpeo_password().equals(encryNewPw)) {
                result.setErrMsg("新旧密码不能相同");
                result.setRetCode(Result.RECODE_ERROR);
                return result;
            }
            peopleEntity.setModuser(UserUtil.getUserId());
            peopleEntity.setEpeo_password(encryNewPw);
            result.setData(ormService.updateSelective(StringUtils.preUpdateEntity(peopleEntity)));
        } catch (Exception e) {
    ...
    public Result mailCode(String mail) {
        ...
        try {
            ...
            RedisUtils.getInstance().setValue(Constant.LOGIN_AUTHENTICATION_ + mail, String.valueOf(randNum), 15 * 60);
            //emailVo.setMailContent("您此次邮箱修改验证码为：" + randNum + ", 有效时间为15分钟，请妥善保管。");
    ...
    public Result modMail(UserInfo userInfo) {
        ...
        try {
            ...
            RedisUtils.getInstance().delKey(Constant.LOGIN_AUTHENTICATION_ + userInfo.getEpeoMail());
            peopleEntity.setModuser(UserUtil.getUserId());
